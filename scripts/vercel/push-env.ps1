...
try {
  ...
  foreach ($envName in $Environments) {
    Write-Host "Setting DATABASE_URL for $envName ..." -ForegroundColor Yellow
    # Pipe the value into Vercel CLI; requires prior `npx vercel login` and `npx vercel link` in this folder
    $dbUrl | npx vercel env add DATABASE_URL $envName
  }
}
finally {
  Pop-Location
