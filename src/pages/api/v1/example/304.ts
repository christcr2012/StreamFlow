...
// BINDER5_FULL.md API Endpoint Example 304
const Example304Schema = z.object({
  id: z.string(),
  payload: z.string(),
});
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const validation = Example304Schema.safeParse(req.body);
    if (!validation.success) {
      return res.status(400).json({
        error: '400_INVALID_304',
    ...
    const exampleRecord = await prisma.note.create({
      data: {
        orgId,
        entityType: 'example_304',
        entityId: id,
        userId: req.headers['x-user-id'] as string || 'system',
        body: `EXAMPLE 304: ${payload}`,
        isPinned: false,
      },
    });

    await auditService.logBinderEvent({
      action: 'api.example.304',
      tenantId: orgId,
      path: req.url,
      ts: Date.now(),
    });
    ...
    console.error('Error in API Endpoint Example 304:', error);
