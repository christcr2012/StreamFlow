...
// BINDER5_FULL.md API Endpoint Example 321
const Example321Schema = z.object({
  id: z.string(),
  payload: z.string(),
});
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const validation = Example321Schema.safeParse(req.body);
    if (!validation.success) {
      return res.status(400).json({
        error: '400_INVALID_321',
    ...
    const exampleRecord = await prisma.note.create({
      data: {
        orgId,
        entityType: 'example_321',
        entityId: id,
        userId: req.headers['x-user-id'] as string || 'system',
        body: `EXAMPLE 321: ${payload}`,
        isPinned: false,
      },
    });

    await auditService.logBinderEvent({
      action: 'api.example.321',
      tenantId: orgId,
      path: req.url,
      ts: Date.now(),
    });
    ...
    console.error('Error in API Endpoint Example 321:', error);
