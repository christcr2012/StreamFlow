...
// BINDER5_FULL.md API Endpoint Example 738
const Example738Schema = z.object({
  id: z.string(),
  payload: z.string(),
});
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const validation = Example738Schema.safeParse(req.body);
    if (!validation.success) {
      return res.status(400).json({
        error: '400_INVALID_738',
    ...
    const exampleRecord = await prisma.note.create({
      data: {
        orgId,
        entityType: 'example_738',
        entityId: id,
        userId: req.headers['x-user-id'] as string || 'system',
        body: `EXAMPLE 738: ${payload}`,
        isPinned: false,
      },
    });

    await auditService.logBinderEvent({
      action: 'api.example.738',
      tenantId: orgId,
      path: req.url,
      ts: Date.now(),
    });
    ...
    console.error('Error in API Endpoint Example 738:', error);
