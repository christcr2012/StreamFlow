...
// BINDER5_FULL.md API Endpoint Example 106
const Example106Schema = z.object({
  id: z.string(),
  payload: z.string(),
});
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const validation = Example106Schema.safeParse(req.body);
    if (!validation.success) {
      return res.status(400).json({
        error: '400_INVALID_106',
    ...
    const exampleRecord = await prisma.note.create({
      data: {
        orgId,
        entityType: 'example_106',
        entityId: id,
        userId: req.headers['x-user-id'] as string || 'system',
        body: `EXAMPLE 106: ${payload}`,
        isPinned: false,
      },
    });

    await auditService.logBinderEvent({
      action: 'api.example.106',
      tenantId: orgId,
      path: req.url,
      ts: Date.now(),
    });
    ...
    console.error('Error in API Endpoint Example 106:', error);
