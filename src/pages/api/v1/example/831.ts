...
// BINDER5_FULL.md API Endpoint Example 831
const Example831Schema = z.object({
  id: z.string(),
  payload: z.string(),
});
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const validation = Example831Schema.safeParse(req.body);
    if (!validation.success) {
      return res.status(400).json({
        error: '400_INVALID_831',
    ...
    const exampleRecord = await prisma.note.create({
      data: {
        orgId,
        entityType: 'example_831',
        entityId: id,
        userId: req.headers['x-user-id'] as string || 'system',
        body: `EXAMPLE 831: ${payload}`,
        isPinned: false,
      },
    });

    await auditService.logBinderEvent({
      action: 'api.example.831',
      tenantId: orgId,
      path: req.url,
      ts: Date.now(),
    });
    ...
    console.error('Error in API Endpoint Example 831:', error);
