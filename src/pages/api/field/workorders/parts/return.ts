...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const validation = ReturnPartSchema.safeParse(req.body);
    ...
    // Create parts return record using Note model
    const partsReturn = await prisma.note.create({
      data: {
        orgId,
        entityType: 'workorder',
        entityId: workOrderId,
        userId,
        body: `Parts Returned: ${inventoryItem.name} - Returned ${payload.quantity} units. Reason: ${payload.return_reason}${payload.notes ? `. Notes: ${payload.notes}` : ''}`,
        isPinned: true, // Pin parts return notes
      },
    });

    await auditService.logBinderEvent({
      action: 'workorder.parts.return',
      tenantId: orgId,
      path: req.url,
      ts: Date.now(),
    });
    ...
    const returnId = `RETURN-${partsReturn.id.substring(0, 6)}`;
    ...
    return res.status(201).json({
      ...
      parts_return: {
        ...
        notes: payload.notes,
        returned_by: userId,
        returned_at: partsReturn.createdAt,
        location: payload.location,
      },
      audit_id: `AUD-RETURN-${partsReturn.id.substring(0, 6)}`,
    });
  } catch (error) {
