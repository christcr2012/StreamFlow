...
async function handler(
  ...
  try {
    // GET - Get task by ID
    if (req.method === 'GET') {
      const result = await taskService.getById(orgId, taskId);
    ...
    // PUT - Update task
    if (req.method === 'PUT') {
      const result = await taskService.update(orgId, userId, taskId, req.body);
      res.status(200).json(result);
      return;
    }
    ...
    if (req.method === 'DELETE') {
      await taskService.delete(orgId, userId, taskId);
      res.status(204).end();
      return;
    }
