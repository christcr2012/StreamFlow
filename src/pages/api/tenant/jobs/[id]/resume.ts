...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const validated = ResumeJobSchema.parse(req.body.payload || req.body);
    ...
    if (job.status !== 'paused') {
      res.status(400).json({
        error: 'InvalidStatus',
        message: `Cannot resume job in ${job.status} status`,
      });
      return;
    }
