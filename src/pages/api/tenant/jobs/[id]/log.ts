// src/pages/api/tenant/jobs/[id]/log.ts
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const log = await jobTicketService.logAction(orgId, userId, role, ticketId, req.body);
    res.status(201).json(log);
    return;
  } catch (error) {
    console.error('Job log API error:', error);
