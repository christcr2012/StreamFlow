// src/pages/api/tenant/billing/prepay.ts
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const result = await creditService.prepay(orgId, userId, req.body);
    res.status(200).json(result);
    return;
  } catch (error) {
    console.error('Prepay API error:', error);
