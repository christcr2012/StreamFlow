// src/pages/api/tenant/ai/jobs/[id]/summary.ts
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const result = await aiJobService.generateSummary(orgId, userId, role, ticketId);
    res.status(200).json(result);
    return;
  } catch (error) {
    console.error('AI job summary API error:', error);
