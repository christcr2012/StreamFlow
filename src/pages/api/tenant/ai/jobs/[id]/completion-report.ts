// src/pages/api/tenant/ai/jobs/[id]/completion-report.ts
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const result = await aiJobService.generateCompletionReport(orgId, userId, role, ticketId);
    res.status(200).json(result);
    return;
  } catch (error) {
    console.error('AI completion report API error:', error);
