...
async function handler(
  ...
  try {
    // GET - List contacts
    if (req.method === 'GET') {
      const { page, limit, search, organizationId, status, sortBy, sortOrder } = req.query;

      const result = await contactService.list(orgId, {
    ...
    // POST - Create contact
    if (req.method === 'POST') {
      const result = await contactService.create(orgId, userId, req.body);
