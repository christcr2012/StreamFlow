...
import {
  STAFF_ROLE_TEMPLATE,
  STAFF_ROLE_VARIANTS,
  StaffRoleCapabilities,
  StaffRoleConstraints,
} from "@/lib/roles/staff-role-template";
...
async function getStaffTemplate(
  req: NextApiRequest,
  res: NextApiResponse,
  user: any,
) {
...
async function getStaffVariants(
  req: NextApiRequest,
  res: NextApiResponse,
  user: any,
) {
  // Get available Staff role variants
  const variants = Object.entries(STAFF_ROLE_VARIANTS).map(
    ([key, variant]) => ({
      id: key,
      ...variant,
      usageStats: {
...
async function getCurrentStaffConfig(
  req: NextApiRequest,
  res: NextApiResponse,
  user: any,
) {
  ...
  const currentPermissions = staffRole.rolePerms.map(
  ...
  const analysis = {
    permissionDifferences: {
      ...
      removed: templatePermissions.filter(
  ...
  res.json({
    ...
    analysis,
    customizationRecommendations: generateStrategicRecommendations(
      analysis,
      [],
    ),
  });
...
async function getCustomizationOptions(
  ...
  const options = {
    ...
    availableExpansions: {
      customerAccess: [
        {
          code: "customer:read_department",
          description: "View all customers in department",
        },
        {
          code: "customer:read_all",
          description: "View all organization customers",
        },
      ...
      taskManagement: [
        ...
        {
          code: "task:assign_others",
          description: "Assign tasks to other staff",
        },
        { code: "task:delete", description: "Delete tasks (with approval)" },
        {
          code: "task:bulk_operations",
          description: "Perform bulk task operations",
        },
      ],
      analytics: [
        {
          code: "analytics:view_personal",
          description: "View personal performance analytics",
        },
        {
          code: "analytics:view_team",
    ...
    constraintOptions: {
      dataVisibility: [
        {
          value: "assigned_only",
          label: "Assigned entities only",
          description: "Staff can only see assigned tasks, customers, etc.",
        },
        {
          value: "department",
          label: "Department scope",
          description: "Staff can see all data within their department",
        },
        {
          value: "territory",
          label: "Territory scope",
          description: "Staff can see data within assigned territory",
        },
        {
          value: "project",
          label: "Project scope",
          description: "Staff can see data within assigned projects",
        },
      ],
      timeRestrictions: [
        { value: "none", label: "No restrictions", description: "24/7 access" },
        {
          ...
          description: "Access only during configured business hours",
        },
        {
          value: "custom_schedule",
          label: "Custom schedule",
          description: "Define specific days and hours",
        },
        {
          value: "shift_based",
          label: "Shift-based",
          description: "Access only during assigned shifts",
        },
      ],
      approvalWorkflows: [
        {
...
async function getStaffRoleOverview(
  req: NextApiRequest,
  res: NextApiResponse,
  user: any,
) {
  ...
  const overview = {
    ...
    activeUsers:
      staffUsers?.filter((ur) => ur.user.status === "active").length || 0,
    customizationLevel: staffRole
      ? calculateCustomizationLevel(staffRole)
      : "none",
...
async function handlePost(
  ...
  switch (action) {
    ...
    case "apply-preset":
      // PHASE 2: Blocked by dependencies
      // Blocked by:
      // - [feature] role/permission system finalization
      // - [feature] audited preset application workflow
      // - [prisma_model] Role, RoleVersion, Permission
      return res
        .status(501)
        .json({ error: "Apply preset pending Phase 2 RBAC enhancements" });
    case "request-elevation":
      // PHASE 2: Blocked by dependencies
      // Blocked by:
      // - [feature] temporary elevation workflow + approvals
      // - [feature] audit logging + notifications
      // - [feature] auth session re-issuance with scoped TTL
      return res
        .status(501)
        .json({
          error: "Temporary elevation pending Phase 2 authorization workflow",
        });
    default:
...
async function createStaffFromTemplate(
  req: NextApiRequest,
  res: NextApiResponse,
  user: any,
) {
  ...
  const result = await db.$transaction(async (tx) => {
    ...
    if (applyConstraints && customizations.constraints?.dataVisibility) {
      const scopeRules = generateScopeRules(
        customizations.constraints.dataVisibility,
        permissions,
      );
...
async function cloneStaffVariant(
  req: NextApiRequest,
  res: NextApiResponse,
  user: any,
) {
  ...
  if (
    !variant ||
    !STAFF_ROLE_VARIANTS[variant as keyof typeof STAFF_ROLE_VARIANTS]
  ) {
  ...
  const result = await db.$transaction(async (tx) => {
    const role = await tx.rbacRole.create({
      data: {
        ...
        slug: (newRoleName || variantConfig.name)
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, "_"),
        isSystem: false,
      },
  ...
  await auditAction(req, {
    ...
    details: {
      ...
      permissionCount:
        STAFF_ROLE_TEMPLATE.basePermissions.length +
        (variantConfig.additionalPermissions?.length || 0),
    },
  });
...
function calculateCustomizationLevel(
  ...
  const permissionDifference = Math.abs(
    currentPermissionCount - basePermissionCount,
  );
  ...
  if (dataVisibility.byDepartment) {
    rules.push({
      ...
      permissions: permissions.filter(
        (p) => p.includes("read") || p.includes("update"),
      ),
...
function generateImmediateRecommendations(role: any, users: any[]): string[] {
  ...
  if (!role) {
    recommendations.push(
      "Create a Staff role using the template to enable employee access",
    );
  }

  if (users && users.length === 0) {
    recommendations.push(
      "Assign the Staff role to employees who need system access",
    );
  }

  if (role && !role.scopes?.length) {
    recommendations.push(
      "Consider adding data visibility scoping for better security",
    );
  }
...
function generateStrategicRecommendations(role: any, users: any[]): string[] {
  ...
  if (users && users.length > 10) {
    recommendations.push(
      "Consider creating specialized role variants for different staff functions",
    );
  }

  recommendations.push(
    "Set up periodic role reviews to ensure permissions remain appropriate",
  );
  recommendations.push(
    "Enable temporary elevation for occasional privileged tasks",
  );

  return recommendations;
}

function calculateScopeComplexity(
  scopes: any[],
