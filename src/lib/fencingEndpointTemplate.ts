...
export function createFencingEndpoint(featureName: string) {
  async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
      ...
      const existing = await prisma.fencing_events.findFirst({
        where: { 
      ...
      // Store event
      await prisma.fencing_events.create({
        data: {
          tenant_id: validated.tenantId,
          feature: featureName,
          request_id: validated.requestId,
