...
function getOAuthConfig(reqOrigin?: string) {
  ...
  const base = reqOrigin || process.env.NEXT_PUBLIC_PROVIDER_URL || process.env.NEXT_PUBLIC_APP_URL;
...
export async function sendGmail(params: { to: string; subject: string; html?: string; text?: string; fromName?: string; reqOrigin?: string }) {
...
export async function exchangeCodeForTokens(code: string, reqOrigin?: string): Promise<{ access_token: string; refresh_token?: string; id_token?: string }>{
