...
import type { Prisma } from "@prisma/client-provider";
...
export default async function PricingManagementPage() {
  ...
  // Fetch all pricing plans with features and history (with build-time guard)
  type PlanWithRelations = Prisma.MarketingPricingPlanGetPayload<{
    include: {
      features: true;
      _count: {
        select: { history: true };
  ...
  let plans: PlanWithRelations[] = [];
  try {
    plans = await prisma.marketingPricingPlan.findMany({
      include: {
        features: {
    ...
    console.log(
      "PricingManagementPage: Database not available during build, using empty data",
    );
    // plans remains empty array
  }
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-blue-800 dark:text-blue-200">
              Changes to pricing plans follow a{" "}
              <strong>Draft → Review → Publish</strong> workflow. Only{" "}
              <strong>PUBLISHED</strong> plans appear on the marketing site. The
              site automatically updates within <strong>60 seconds</strong> of
              publishing (no deployment needed).
            </p>
          </div>
        </div>
      </div>
