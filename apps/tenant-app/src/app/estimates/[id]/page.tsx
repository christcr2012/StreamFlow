...
export default async function EstimateDetailPage({
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_PATH ?? ''}/api/estimates`, {
    cache: 'no-store',
  });
  const data = (await res.json()) as { estimates: Estimate[] };
  const estimate = data.estimates.find((e) => e.id === params.id);

  if (!estimate) {
    return (
      <div className="max-w-4xl mx-auto p-6">
        <h1 className="text-2xl font-bold text-gray-900">Estimate not found</h1>
        <p className="mt-2 text-gray-600">This is a Phase 1 stub detail page. Create the estimate detail API in Phase 2.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">Estimate {estimate.publicId}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="bg-white border rounded-lg p-4">
          ...
          <p className="text-gray-700">Status: <span className="font-medium capitalize">{estimate.status}</span></p>
          <p className="text-gray-700">Valid Until: {new Date(estimate.validUntil).toLocaleDateString()}</p>
          <p className="text-gray-900 font-semibold mt-2">Total: ${estimate.total.toFixed(2)}</p>
        </div>
      </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {estimate.lineItems.map((li) => (
              <tr key={li.id}>
                <td className="px-6 py-4 text-sm text-gray-900">{li.description}</td>
                <td className="px-6 py-4 text-sm text-right text-gray-700">{li.quantity}</td>
                <td className="px-6 py-4 text-sm text-right text-gray-700">${li.unitPrice.toFixed(2)}</td>
                <td className="px-6 py-4 text-sm text-right font-semibold text-gray-900">${li.total.toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
