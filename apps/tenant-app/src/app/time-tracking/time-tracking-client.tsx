...
export function TimeTrackingClient({ orgId }: { orgId: string }) {
  ...
  const totalPay = timeEntries
    .filter((e) => e.status === "approved")
    .reduce((s, e) => s + e.totalPay, 0);
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-blue-700">{activeEntry.jobTitle}</p>
              ...
              {timeEntries.map((entry) => (
                <tr key={entry.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      className={`px-2 py-1 text-xs font-semibold rounded-full ${entry.status === "approved" ? "bg-green-100 text-green-800" : entry.status === "active" ? "bg-blue-100 text-blue-800" : "bg-yellow-100 text-yellow-800"}`}
                    >
                      {entry.status}
                    </span>
                  </td>
        ...
        <div className="mt-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-yellow-800">
              ...
              <p>
                Time tracking with stub data. Blocked by real clock in/out
                endpoints, GPS verification, payroll calculations.
              </p>
            </div>
          </div>
        </div>
