...
// Phase 1: Subscription tier and usage display
...
export function SubscriptionClient({ orgId }: SubscriptionClientProps) {
  ...
  const tierInfo = TIER_INFO[subscription.tierKey as keyof typeof TIER_INFO] || TIER_INFO.starter;
  const TierIcon = tierInfo.icon;

  const usagePercentages = {
    users: (subscription.usage.activeUsers / subscription.limits.maxUsers) * 100,
    locations: (subscription.usage.activeLocations / subscription.limits.maxLocations) * 100,
    jobs: (subscription.usage.jobsThisMonth / subscription.limits.maxJobsPerMonth) * 100,
    storage: (subscription.usage.storageUsedGb / subscription.limits.storageGb) * 100,
  };

  return (
    <div className="space-y-6">
      {/* Current Plan Card */}
      <div className={`bg-gradient-to-br from-${tierInfo.color}-50 to-${tierInfo.color}-100 dark:from-${tierInfo.color}-900/20 dark:to-${tierInfo.color}-800/20 border-2 border-${tierInfo.color}-200 dark:border-${tierInfo.color}-800 rounded-lg p-6`}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className={`text-2xl font-bold text-${tierInfo.color}-900 dark:text-${tierInfo.color}-100`}>
              ...
              <p className={`text-sm text-${tierInfo.color}-700 dark:text-${tierInfo.color}-300 mt-1`}>
      ...
      <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="text-sm text-yellow-800 dark:text-yellow-200">
            ...
            <p>This page shows example subscription data. Upgrade/downgrade functionality will be enabled in Phase 2 with Stripe integration.</p>
          </div>
        </div>
      </div>
    </div>
...
function UsageCard({ icon: Icon, label, current, limit, percentage, color, unit = '' }: UsageCardProps) {
...
function TierCard({ tierKey, name, price, features, current, popular }: TierCardProps) {
  return (
    <div className={`relative rounded-lg border-2 p-6 ${
      ...
      <div className="text-center mb-6">
        ...
        {price !== 'Custom' && <div className="text-sm text-gray-600 dark:text-gray-400">/month</div>}
      </div>
